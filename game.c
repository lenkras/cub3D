...
static void	view_set_angle(t_cub *cub, const char c)
{
	if (c == 'E')
		cub->gaze = 0.0f * M_PI;
	else if (c == 'S')
...
static void	find_position(t_cub *cub)
{
	cub->p_x = (float)cub->player_w + 0.5f;
	cub->p_y = (float)(cub->height - 1 - cub->player_h) + 0.5f;
	view_set_angle(cub, \
		cub->map[cub->height - 1 - cub->player_h][cub->player_w]);
}

static void	render(t_cub *cub)
{
